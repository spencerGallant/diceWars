...
function start_player(){
	...
	// Hide all sprites that are not relevant for the current player's turn.
    // This loop goes through sprites starting from `sn_info` up to `sn_max`, making them invisible.
    // This could include hiding game messages, options, or other player-specific data not needed at the start of a new turn.
	for( var i=sn_info; i<sn_max; i++ ){
		spr[i].visible = false;
	}

	// Checks if the game is in spectator mode. 
    // Spectator mode is likely a view-only mode where the player cannot interact with the game.
	if ( !spectate_mode ) {
		draw_player_data();
	}else{
	...
	// Determines if the current player is the user (human player).
    // `game.jun[game.ban]` gets the current player's index, `game.user` is the human player's index.
    // If they match, it means it's the human player's turn.
	if( game.jun[game.ban] == game.user || game.jun[game.ban] == game.user1){
		start_man();
	}else{
//		start_man();
		start_com();
...
function after_battle(){
	...
	if( (game.player[game.user].area_tc==0 && game.player[game.user1].area_tc==0) && !spectate_mode){
		draw_player_data();
		start_gameover();
	}else{
		var c=0;
...
function next_player(){
	...
	else if ( game.jun[game.ban] == game.user1 ) playSound("snd_myturn");
